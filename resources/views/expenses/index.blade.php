...
<div
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400">Record and review operating, direct, and other expenses.</p>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-800">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-gray-800 text-sm">
                    @forelse($expenses as $e)
                        <tr>
                            ...
                            <td class="px-5 py-3 text-gray-700 dark:text-gray-300">{{ strtoupper($e->method) }}</td>
                            <td class="px-5 py-3 text-gray-700 dark:text-gray-300">{{ $e->reference ?? '—' }}</td>
                            <td class="px-5 py-3 text-right font-medium text-gray-900 dark:text-gray-100">RWF {{ $fmt($e->amount) }}</td>
                            <td class="px-5 py-3 text-gray-600 dark:text-gray-300">
                                <span class="line-clamp-2">{{ Str::limit($e->note ?? '—', 80) }}</span>
                            </td>
                            <td class="px-5 py-3">
                                <div class="flex justify-end gap-1">
                                    @role('admin|manager|accountant')
                                        ...
                                        <form x-on:submit.prevent="openDel($el.action, 'Expense #{{ $e->id }}')" method="POST" action="{{ route('expenses.destroy', $e) }}">
                                            @csrf @method('DELETE')
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="px-5 py-6 text-center text-gray-500 dark:text-gray-400">No expenses found.</td>
                        </tr>
                    @endforelse
                </tbody>
                ...
                <tfoot class="bg-gray-50 dark:bg-gray-800/50 text-sm">
                    <tr>
                        ...
                        <td class="px-5 py-3 text-right font-semibold text-gray-900 dark:text-gray-100">RWF {{ $fmt($pageTotal ?? 0) }}</td>
                        <td colspan="2"></td>
                    </tr>
                </tfoot>
            </table>
