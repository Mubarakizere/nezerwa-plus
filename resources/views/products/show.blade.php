...
<div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
    ...
    <div class="flex flex-col md:flex-row items-start md:items-center justify-between gap-3">
        ...
        <div class="flex flex-wrap gap-2">
            ...
            <a href="{{ route('stock.history', ['product_id' => $product->id]) }}" class="btn btn-secondary text-sm flex items-center gap-1">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <x-stat-card title="Margin" value="{{ $fmt2($margin) }}%" color="{{ $margin >= 0 ? 'emerald' : 'rose' }}" />
        <x-stat-card title="Current Stock" value="{{ $fmt0($current) }}"
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm overflow-hidden mt-4">
        <div class="p-4 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
            ...
            <a href="{{ route('stock.history', ['product_id' => $product->id]) }}"
               class="text-indigo-600 dark:text-indigo-400 text-sm hover:underline">View All →</a>
        </div>
                    ...
                    @forelse($product->stockMovements as $move)
                        @php
                            ...
                            if ($move->source_type === Purchase::class) {
                                $src = ['label' => "Purchase #{$move->source_id}", 'url' => Route::has('purchases.show') ? route('purchases.show', $move->source_id) : null, 'class' => 'text-indigo-600 dark:text-indigo-400'];
                            } elseif ($move->source_type === Sale::class) {
                                $src = ['label' => "Sale #{$move->source_id}", 'url' => Route::has('sales.show') ? route('sales.show', $move->source_id) : null, 'class' => 'text-green-600 dark:text-green-400'];
                            } elseif ($move->source_type === PurchaseReturn::class) {
                                // Try common route name variants
                                $prRoute = null;
                                ...
                                $src = ['label' => 'Manual Adjustment', 'url' => null, 'class' => 'text-gray-500 dark:text-gray-400'];
                            }
                        @endphp
                        ...
                        <tr class="hover:bg-gray-50 dark:hover:bg-gray-800/60 transition-all">
                            ...
                            <td class="px-4 py-3">
                                @if($src && $src['url'])
                                    <a href="{{ $src['url'] }}" class="{{ $src['class'] }} hover:underline">{{ $src['label'] }}</a>
                                @elseif($src)
