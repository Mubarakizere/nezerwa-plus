...
@section('content')
@php
    $parents = $parents ?? \App\Models\Category::where('id','!=',$category->id)->orderBy('name')->get(['id','name','kind']);
@endphp
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-6">
    ...
    {{-- Form Card --}}
    <div x-data="{
            kind: '{{ old('kind', $category->kind) }}',
            color: '{{ old('color', $category->color ?? '#22c55e') }}',
            icon: '{{ old('icon', $category->icon ?? 'folder') }}',
        ...
        <form action="{{ route('categories.update', $category) }}" method="POST" class="space-y-5">
            ...
            <div>
                ...
                <textarea name="description" rows="3" class="form-textarea w-full">{{ old('description', $category->description) }}</textarea>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" name="code" value="{{ old('code', $category->code) }}" class="form-input w-full">
                    @error('code') <p class="text-red-600 text-sm mt-1">{{ $message }}</p> @enderror
                </div>
                <div>
                    <label class="form-label">Sort Order</label>
                    <input type="number" name="sort_order" value="{{ old('sort_order', $category->sort_order ?? 0) }}" class="form-input w-full">
...
document.addEventListener('input', () => { if (window.lucide) lucide.createIcons(); });
</script>
@endpush
@endsection
