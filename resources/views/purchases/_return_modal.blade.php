...
@push('scripts')
<script>
// Define this FIRST, before Alpine tries to use it
    ...
    return {
        ...
        init() {
            console.log('🚀 Modal initialized', {
                ...
                shouldOpen: ctx.shouldOpen
            });

            // Initialize lines with proper defaults
            this.lines = (ctx.lines || []).map(r => ({
                ...
                return_qty: 0, // Start at 0
                unit_cost: Number(r.unit_cost || 0)
            }));

            this.form = { ...(ctx.initial || {}) };
            this.recalc();

            // ONLY open if there were validation errors
            if (ctx.shouldOpen === true) {
                console.log('⚠️ Opening modal due to validation errors');
                this.open = true;
            }
        },
        ...
        get valid() {
            ...
            if (refund > this.total + 0.01) {
                return false;
            }

            // If refund > 0, must have payment channel
            if (refund > 0 && !this.form.payment_channel) {
                return false;
            }
...
@php
    ...
    // CRITICAL: Only auto-open when there are VALIDATION errors from the purchaseReturn bag
    $hasValidationErrors = $errors->purchaseReturn->any();
    $shouldOpen = $hasValidationErrors;

    // Prepare the context object properly (fix JSON syntax)
    $modalContext = [
        'lines' => $rows->toArray(),
        'initial' => [
...
>
    ...
    <form method="POST"
        ...
        {{-- Display ALL errors from purchaseReturn bag --}}
        ...
        @if($rows->isEmpty())
            <div class="rounded-lg border-2 border-yellow-400 bg-yellow-50 text-yellow-800 text-sm px-4 py-3">
                ...
                <p class="mt-1">All items from this purchase have already been fully returned.</p>
            </div>
        @endif
            ...
            <table class="min-w-full text-sm divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-300 uppercase text-xs font-medium">
                    <tr>
                        ...
                        <th class="px-4 py-3 text-right">Return Qty <span class="text-red-500">*</span></th>
                        <th class="px-4 py-3 text-right">Unit Cost <span class="text-red-500">*</span></th>
                    ...
                    <template x-for="(row, idx) in lines" :key="row.purchase_item_id">
                        <tr :class="rowError(idx) ? 'bg-red-50 dark:bg-red-900/20' : ''">
                            <td class="px-4 py-3">
                                ...
                                <span x-text="row.product_name" class="font-medium text-gray-900 dark:text-gray-100"></span>
                            </td>

                            <td class="px-4 py-3 text-right text-gray-600 dark:text-gray-400" x-text="fmt(row.purchased_qty)"></td>
                            <td class="px-4 py-3 text-right text-gray-600 dark:text-gray-400" x-text="fmt(row.returned_qty)"></td>
                            <td class="px-4 py-3 text-right font-medium text-gray-900 dark:text-gray-100" x-text="fmt(row.remaining)"></td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <input type="number"
                                       ...
                                       :class="rowError(idx) ? 'border-red-500 bg-red-50' : ''"
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div class="bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-900 dark:to-gray-800 rounded-lg p-4 border border-gray-200 dark:border-gray-700 space-y-3">
                <div class="flex justify-between items-center text-sm font-medium pb-2 border-b">
                    <span class="text-gray-600 dark:text-gray-400">Total Return Value:</span>
                    <span class="text-lg font-bold text-gray-900 dark:text-gray-100" x-text="'RWF ' + money(total)"></span>
                </div>

                <div>
                    <label class="text-sm font-medium text-gray-700 dark:text-gray-300 block mb-1">Refund Amount</label>
                    <input type="number"
                           step="0.01"
                           min="0"
                           name="refund_amount"
                ...
                <div class="text-xs text-gray-600 dark:text-gray-400 bg-blue-50 dark:bg-blue-900/20 p-2 rounded border border-blue-200 dark:border-blue-800">
                    <strong>Note:</strong> Refund amount cannot exceed total return value.
                    Leave at 0 for a credit note only.
                </div>
            </div>
        </div>
        ...
        <div class="flex justify-between items-center gap-3 pt-4 border-t">
            <div class="text-sm">
                <span x-show="!valid" class="text-red-600 font-medium">⚠️ Please enter return quantities before submitting</span>
                <span x-show="valid" class="text-green-600 font-medium">✓ Ready to submit</span>
            </div>
