@php
    ...
    if ($isEdit && ($purchase->subtotal ?? 0) > 0) {
        $taxPct      = round(($purchase->tax / $purchase->subtotal) * 100, 2);
        $discountPct = round(($purchase->discount / $purchase->subtotal) * 100, 2);
    }
    ...
    if ($initialLines->isEmpty()) $initialLines = collect([['product_id'=>'', 'quantity'=>1, 'unit_cost'=>0]]);

    $initialState = [
        'supplier_id'     => old('supplier_id', $isEdit ? $purchase->supplier_id : ''),
        'purchase_date'   => old('purchase_date', $isEdit ? $purchase->purchase_date->format('Y-m-d') : now()->format('Y-m-d')),
        'payment_channel' => old('payment_channel', $isEdit ? $purchase->payment_channel : 'cash'),
...
<div x-data="purchaseForm(@js($initialState))" x-init="init()" class="space-y-6">
    ...
    <section class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-5 grid grid-cols-1 md:grid-cols-4 gap-5">
        ...
        <div>
            ...
            @error('purchase_date') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <x-label value="Payment Channel" />
            {{-- ✅ This is the field the controller reads --}}
            ...
            <div class="flex gap-2 mt-2">
                <button type="button" @click="setChannel('cash')" class="px-2 py-1 rounded-md border" :class="badgeClass('cash')">Cash</button>
                <button type="button" @click="setChannel('bank')" class="px-2 py-1 rounded-md border" :class="badgeClass('bank')">Bank</button>
                <button type="button" @click="setChannel('momo')" class="px-2 py-1 rounded-md border" :class="badgeClass('momo')">MoMo</button>
            </div>
            @error('payment_channel') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
        </div>

        <div>
            <x-label value="Reference (optional)" />
            {{-- ✅ Stored in purchases.method (reference/cheque/txn id) --}}
            <input type="text" name="method" x-model="state.method" class="form-input" placeholder="Invoice # / Txn ID / Cheque no.">
                    ...
                    <template x-for="(row, idx) in state.lines" :key="row.key">
                        <tr>
                            <td class="px-4 py-2">
                                ...
                                @error('products.*.product_id') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                            </td>
                            <td class="px-4 py-2 text-right">
                                <input type="number" step="1" min="1"
                                       ...
                                       @input="recalc()">
                                @error('products.*.quantity') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                            </td>
                            <td class="px-4 py-2 text-right">
                                <input type="number" step="0.01" min="0"
                                       ...
                                       @input="recalc()">
                                @error('products.*.unit_cost') <p class="text-xs text-red-600 mt-1">{{ $message }}</p> @enderror
                            </td>
    ...
    <section class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="md:col-span-2">
            ...
            <textarea name="notes" x-model="state.notes" rows="4" class="form-textarea" placeholder="Any remarks..."></textarea>
        ...
        <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-4 space-y-3">
            ...
            <div class="flex items-center justify-between gap-3 text-sm">
                ...
                <input type="number" step="0.01" min="0" max="100" name="tax" class="form-input text-right w-24"
                       x-model.number="state.tax" @input="recalc()">
            </div>
            <div class="flex justify-between text-sm">
            ...
            <div class="flex items-center justify-between gap-3 text-sm">
                ...
                <input type="number" step="0.01" min="0" max="100" name="discount" class="form-input text-right w-24"
                       x-model.number="state.discount" @input="recalc()">
            </div>
            <div class="flex justify-between text-sm">
            ...
            <div class="flex items-center justify-between gap-3 text-sm">
                ...
                <div class="flex items-center gap-2">
                    <input type="number" step="0.01" min="0" id="amount_paid" name="amount_paid"
                           ...
                           x-model.number="state.amount_paid" @input="recalc()">
    ...
    <div class="pt-2 flex flex-col sm:flex-row gap-2">
        <button type="submit" class="btn btn-success flex-1 flex items-center justify-center gap-1">
            <i data-lucide="save" class="w-4 h-4"></i> {{ $isEdit ? 'Update Purchase' : 'Save Purchase' }}
        </button>
        <a href="{{ $isEdit ? route('purchases.show',$purchase) : route('purchases.index') }}" class="btn btn-outline flex-1 flex items-center justify-center gap-1">
...
function purchaseForm(initial){
    const withKeys = (arr)=> arr.map(r => ({ key: (crypto?.randomUUID?.() || (Date.now()+Math.random())), ...r }));
    return {
        state: {
            ...
            lines: withKeys(Array.isArray(initial.lines) ? initial.lines : [{product_id:'',quantity:1,unit_cost:0}]),
        },
        subtotal: 0, taxValue: 0, discountValue: 0, grand: 0, balance: 0,

        init(){ this.recalc(); },
        setChannel(c){ this.state.payment_channel = c; },
        badgeClass(c){
        ...
        addLine(){ this.state.lines.push({ key: (crypto?.randomUUID?.() || (Date.now()+Math.random())), product_id:'', quantity:1, unit_cost:0 }); },
        clearLines(){ this.state.lines = []; this.recalc(); },
        removeLine(i){ this.state.lines.splice(i,1); this.recalc(); },
        ...
        lineTotal(r){ return (Number(r.quantity||0) * Number(r.unit_cost||0)); },
        recalc(){
        ...
        payFull(){ this.state.amount_paid = this.grand; this.recalc(); },
