...
@php
    ...
    $oldType        = old('type', $entry->type ?? '');
    $oldCustomerId  = old('customer_id', $entry->customer_id ?? '');
    $oldSupplierId  = old('supplier_id', $entry->supplier_id ?? '');
@endphp
...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8"
     x-data="{
        ...
        onCustomerPick(){ if (this.customerId) this.supplierId=''; },
        onSupplierPick(){ if (this.supplierId) this.customerId=''; }
     }"
>
    {{-- Header --}}
    ...
    <div class="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-xl shadow-sm p-6">
        <form action="{{ route('debits-credits.update', $entry->id) }}" method="POST" class="space-y-6">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="type" class="form-select" x-model="type" @change="onTypeChange" required>
                        <option value="debit"  @selected($oldType==='debit')>Debit (Money Out)</option>
                        <option value="credit" @selected($oldType==='credit')>Credit (Money In)</option>
                    </select>
                    <p class="text-xs text-gray-500 mt-1" x-show="type==='debit'">Tip: Debits usually relate to a Supplier.</p>
                    <p class="text-xs text-gray-500 mt-1" x-show="type==='credit'">Tip: Credits usually relate to a Customer.</p>
                ...
                <div>
                    ...
                    <input type="number" step="0.01" min="0.01" name="amount"
                           value="{{ old('amount', $entry->amount) }}" class="form-input" required>
                ...
                <div>
                    ...
                    <input type="date" name="date" value="{{ old('date', $entry->date) }}" class="form-input" required>
                ...
                <div>
                    ...
                    <input type="number" min="1" name="transaction_id"
                           value="{{ old('transaction_id', $entry->transaction_id) }}"
                           class="form-input" placeholder="Transaction ID">
                    <p class="text-xs text-gray-500 mt-1">If this entry corresponds to an existing Transaction, put its ID here.</p>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea name="description" rows="3" class="form-textarea"
                              placeholder="Optional note...">{{ old('description', $entry->description) }}</textarea>
